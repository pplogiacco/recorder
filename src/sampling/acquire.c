...
#include "ADA2200.h"    // HAL ADA2200 SPI1
#include "../daas/libfft.h"
#include "acquire.h"
 ...
 *
 * { <sig_fq>, <sig_maxa>, <adc_fq>, <res_scale>, [<dT>,<a>],[...] }
 * -------------------------------------------------------------------------- */
#define SIG_DEF_AMP 300
#define SIG_MAX_AMP 2048  
...
uint16_t acquireSig(sample_t* dbuf, uint16_t nsec, uint16_t nsamples, uint16_t sig_freq, uint16_t sig_amp, bool add_deltatick) {
    ...
    dTr = sig_freq  * ADC_T;
    kTr =   dTr / _2Pi; // freq. in Hz del segnale  
   //  kTr = _2Pi / (sig_freq * ADC_T); // freq. in Hz del segnale      
    dTr = 0;

    *dbuf = sig_freq; // <sig_fq>
    dbuf++;
    *dbuf = sig_amp; // <sig_maxa>
    dbuf++;
    *dbuf = (1 / ADC_T); // <adc_fq>
    dbuf++;
    *dbuf = SIG_MAX_AMP; // <res_scale>
    dbuf++;
    ibuf = 4;
    ...
    while ((ibuf < nsamples) && ((nT * ADC_T) <= nsec)) {
        ...
        if (add_deltatick) {
            *dbuf = (nT == 0) ? 0 : 1;
            dbuf++;
            *dbuf = (SIG_MAX_AMP / 2) - ((float) sinf(dTr) * sig_amp);
            dbuf++;
            ibuf += 2;
        } else {
            //            return (SIG_MAX_AMP / 2) - ((float) sin(dTr) * sig_amp);
            ibuf++;
        }
...
uint16_t acquireAV_EVC_P2P(sample_t* dbuf, uint16_t nsec, uint16_t dbsize, uint16_t wbsize, uint16_t adc_pr3, uint16_t p2p_filter) {
    ...
    device.cnf.calibration.av_filter = 20;
    n = acquireAV_P2P(wbPtr, 1, wbsize, adc_pr3, device.cnf.calibration.av_filter);
    // n == wbsize    
